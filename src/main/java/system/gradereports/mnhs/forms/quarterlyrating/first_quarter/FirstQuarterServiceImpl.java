...
@Transactional
@AllArgsConstructor
public class FirstQuarterServiceImpl implements IFirstQuarterService{
    private IFirstQuarterRepository firstQuarterRepository;
    ...
    public ResponseEntity<Object> insertGrade(HashMap<String, Object> data) {
        ...
        FirstQuarter priorRecord = firstQuarterRepository.findPriorRecordByStudentIdAndSubjectId(priorStudent.getId(), priorSubject.getId());
        if(priorRecord != null) {
        ...
        firstQuarterRepository.save(new FirstQuarter(priorSubject, grade, priorStudent));
    ...
    public ResponseEntity<Object> updateGrade(HashMap<String, Object> data) {
        ...
        FirstQuarter priorRecord = firstQuarterRepository.findPriorRecord(recordId);
        Student priorStudent = studentService.findStudentByLrn(data.get("lrn").toString());
        Subject priorSubject = subjectService.findSubjectByName(data.get("subjectName").toString().toUpperCase());
        Long grade = Long.parseLong(data.get("grade").toString());
        ...
        response = new HashMap<>();
        response.put("result", "first_quarter_inserted");
        firstQuarterRepository.save(new FirstQuarter(priorRecord ,priorSubject, grade, priorStudent));
        return new ResponseEntity<>(response, HttpStatus.ACCEPTED);
    }
}
